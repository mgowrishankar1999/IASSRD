...
//         (item) =>
//             item.journalName.toLowerCase().includes(searchTerm.toLowerCase()) ||
//             item.journalKey.toLowerCase().includes(searchTerm.toLowerCase())
//     );
...
//         { label: "Journal Key", key: "journalKey" },
//         { label: "ISSN Online", key: "issnOnline" },
//         { label: "ISSN Print", key: "issnPrint" },
...
//     const csvData = filteredJournals.map((item, index) => ({
//         sNo: index + 1,
//         journalName: item.journalName,
//         journalKey: item.journalKey,
//         issnOnline: item.issnOnline,
//         issnPrint: item.issnPrint,
...
//                 axios.delete(`${BASE_URL}/journals/${id}`).then((response) => {
//                     if (!response.data.success) {
//                         throw new Error(`Failed to delete journal with ID ${id}`);
//                     }
...
//                 prevJournals.filter((journal) => !selectedRows.includes(journal.journalId))
//             );
...
//             setSuccess("Journals deleted successfully!");
//             setTimeout(() => setSuccess(null), 3000);

//             setShowDeleteModal(false);
//         } catch (err) {
//             setError("Error deleting journals: " + err.message);
//         } finally {
//             setLoading(false);
//         }
//     };
...
//             const journalToEdit = journals.find((j) => j.journalId === selectedRows[0]);
//             navigate("/journalsaddnew", { state: { journal: journalToEdit } });
//             window.scrollTo(0, 0);
//         }
//     };
...
//                     <h1 className="text-2xl font-bold">Journals Dashboard</h1>
//                     <p className="text-sm opacity-90 mt-1">Manage journals and their details.</p>
//                 </header>
...
//                         <input
//                             type="text"
//                             placeholder="Search by name or journal key..."
//                             value={searchTerm}
...
//                                         onChange={(e) => {
//                                             if (e.target.checked) {
//                                                 setSelectedRows(currentItems.map((item) => item.journalId));
//                                             } else {
//                                                 setSelectedRows([]);
//                                             }
//                                         }}
...
//                                             currentItems.every((item) => selectedRows.includes(item.journalId))
//                                         }
...
//                                 <th className="p-3 text-left">Journal Key</th>
//                                 <th className="p-3 text-left">ISSN Online</th>
//                                 <th className="p-3 text-left">ISSN Print</th>
...
//                                         <input
//                                             type="checkbox"
//                                             checked={selectedRows.includes(item.journalId)}
//                                             onChange={() => handleCheckboxChange(item.journalId)}
//                                             className="h-5 w-5 text-blue-500 border-blue-200 rounded focus:ring-blue-400"
//                                         />
//                                     </td>
//                                     <td className="p-3">{startIndex + index + 1}</td>
//                                     <td className="p-3">{item.journalName}</td>
//                                     <td className="p-3">{item.journalKey}</td>
//                                     <td className="p-3">{item.issnOnline}</td>
//                                     <td className="p-3">{item.issnPrint}</td>
...
//                                 Are you sure you want to delete {selectedRows.length} journal(s)?
//                             </p>
...
const BASE_URL = "https://iassrd.com:8081";
...
const Journals = () => {
    ...
    useEffect(() => {
        const fetchJournals = async () => {
            try {
                const response = await axios.get(`${BASE_URL}/api/v1/journals`);
                if (response.data.success) {
                    setJournals(response.data.data);
                } else {
                    setError("Failed to fetch journals");
    ...
    const csvHeaders = [
        ...
        { label: "ISSN Online", key: "issnOnline" },
        { label: "ISSN Print", key: "issnPrint" },
        { label: "Cover Page", key: "coverPage" },
        { label: "Created User ID", key: "createdUserId" },
        { label: "Created User Type", key: "createdUserType" },
        { label: "Created Date", key: "createdDate" },
        { label: "Updated User ID", key: "updatedUserId" },
    ...
    const confirmDelete = async () => {
        try {
            setLoading(true);
            const deletePromises = selectedRows.map((id) =>
                axios.delete(`${BASE_URL}/api/v1/journals/${id}`).then((response) => {
                    if (!response.data.success) {
                        throw new Error(`Failed to delete journal with ID ${id}`);
                    }
                    return response.data;
