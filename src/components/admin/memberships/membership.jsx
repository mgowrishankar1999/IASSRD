...
const Memberships = () => {
    ...
    const filteredMemberships = memberships.filter((item) => {
        return (
            ...
            (item.membershipType || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
            (item.membershipYear || "").toString().includes(searchTerm)
        );
    });
    ...
    const currentItems = filteredMemberships.slice(startIndex, startIndex + itemsPerPage);
    ...
    const csvHeaders = [
        ...
        { label: "Membership Year", key: "membershipYear" },
        { label: "Name", key: "name" },
        { label: "Affiliation", key: "affiliation" },
        { label: "Specialization", key: "specialization" },
        { label: "Google Scholar Link", key: "googleScholarLink" },
        { label: "ResearchGate Link", key: "researchgateLink" },
        { label: "Academia Link", key: "academiaLink" },
        { label: "ORCID", key: "orcid" },
        { label: "SSRN", key: "ssrn" },
        { label: "GitHub", key: "github" },
        { label: "Institution Profile Link", key: "institutionProfileLink" },
        { label: "Scopus Link", key: "scopusLink" },
        { label: "Web of Science Link", key: "wosLink" },
    ...
    const csvData = filteredMemberships.map((item, index) => ({
        sNo: index + 1,
        membershipType: item.membershipType || "N/A",
        membershipYear: item.membershipYear || "N/A",
        name: item.name || "N/A",
        affiliation: item.affiliation || "N/A",
        specialization: item.specialization || "N/A",
        googleScholarLink: item.googleScholarLink || "N/A",
        researchgateLink: item.researchgateLink || "N/A",
        academiaLink: item.academiaLink || "N/A",
        orcid: item.orcid || "N/A",
        ssrn: item.ssrn || "N/A",
        github: item.github || "N/A",
        institutionProfileLink: item.institutionProfileLink || "N/A",
        scopusLink: item.scopusLink || "N/A",
        wosLink: item.wosLink || "N/A",
        photo: item.photo || "N/A",
        email: item.email || "N/A",
        mobileNumber: item.mobileNumber || "N/A",
        createdUserId: item.createdUserId || "N/A",
        createdDate: new Date(item.createdDate).toLocaleString(),
        updatedUserId: item.updatedUserId || "N/A",
    ...
    const confirmDelete = async () => {
        try {
            ...
            const deletePromises = selectedRows.map((id) =>
                axios.delete(`${BASE_URL}/memberships/${id}`).then((response) => {
                    if (!response.data.success) {
            ...
            setMemberships((prevMemberships) =>
                prevMemberships.filter((membership) => !selectedRows.includes(membership.membershipId))
            );
            ...
            setModal({ show: true, type: "success", message: "Memberships deleted successfully!" });
            setTimeout(() => setModal({ show: false, type: "", message: "" }), 3000);

            setShowDeleteModal(false);
        } catch (err) {
            setModal({ show: true, type: "error", message: "Error deleting memberships: " + err.message });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleEdit = () => {
        if (selectedRows.length === 1) {
            const membershipToEdit = memberships.find((m) => m.membershipId === selectedRows[0]);
            navigate("/addnewmembership", { state: { membership: membershipToEdit } });
            window.scrollTo(0, 0);
        }
    };
        ...
        <div className="flex min-h-screen bg-blue-50 ">
            ...
            <div className="w-screen p-6 pt-20">
                ...
                <header className="bg-cyan-500 text-white p-4 rounded-t-lg mb-4 shadow-md">
                    <h1 className="text-2xl font-bold">Fellow Memberships Dashboard</h1>
                    <p className="text-sm opacity-90 mt-1">Manage memberships and their details.</p>
                </header>
                            ...
                            <tr className="bg-gray-200 text-gray-700 text-sm">
                                <th className="p-3 text-left">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            if (e.target.checked) {
                                                setSelectedRows(currentItems.map((item) => item.membershipId));
                                            } else {
                                                setSelectedRows([]);
                                            }
                                        }}
                                        checked={
                                            ...
                                            currentItems.every((item) => selectedRows.includes(item.membershipId))
                                        }
                            ...
                            {currentItems.map((item, index) => (
                                ...
                                >
                                    <td className="p-3">
                                        <input
                                            type="checkbox"
                                            checked={selectedRows.includes(item.membershipId)}
                                            onChange={() => handleCheckboxChange(item.membershipId)}
                                            className="h-5 w-5 text-blue-500 border-blue-200 rounded focus:ring-blue-400"
                                        />
                                    </td>
                                    <td className="p-3">{startIndex + index + 1}</td>
                                    <td className="p-3">{item.membershipType || "N/A"}</td>
                                    <td className="p-3">{item.membershipYear || "N/A"}</td>
                                    <td className="p-3">{item.name || "N/A"}</td>
                                    <td className="p-3">{item.affiliation || "N/A"}</td>
                                    <td className="p-3">{item.specialization || "N/A"}</td>
                                    ...
                                    {/* <td className="p-3">{item.updatedUserId || "N/A"}</td>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-6 text-gray-600 text-sm gap-4">
                    <span>
                        Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, totalItems)} of {totalItems} memberships
                    </span>
                    ...
                    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                            <p className="text-lg font-semibold text-gray-700 mb-4">
                                Are you sure you want to delete {selectedRows.length} membership(s)?
                            </p>
