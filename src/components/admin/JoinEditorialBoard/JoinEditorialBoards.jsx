...
//     useEffect(() => {
//         const fetchEditorialBoardMembers = async () => {
//             try {
//                 const response = await axios.get(`${BASE_URL}/joineditorialboard`);
//                 setEditorialBoardMembers(response.data); // Assuming the API returns an array
//             } catch (err) {
...
//     // Handle search
//     const filteredMembers = editorialBoardMembers.filter(
//         (item) =>
...
//         { label: "Photo Path", key: "photo" },
//         { label: "CV Path", key: "cv" },
...
//             pdf.save("editorial-board-members.pdf");
//         } catch (error) {
...
//                 axios.delete(`${BASE_URL}/joineditorialboard/${id}`).then((response) => {
...
//             setEditorialBoardMembers((prevMembers) =>
//                 prevMembers.filter((member) => !selectedRows.includes(member.jebId))
//             );
...
//             const memberToEdit = editorialBoardMembers.find((m) => m.jebId === selectedRows[0]);
//             navigate("/addneweditorialboard", { state: { editorialBoardMember: memberToEdit } });
//             window.scrollTo(0, 0);
//         }
//     };
...
//                     <p className="text-sm opacity-90 mt-1">Manage editorial board applications and their details.</p>
//                 </header>
...
//                             data={csvData}
//                             headers={csvHeaders}
//                             filename={"editorial-board-members.csv"}
...
//                                         onChange={(e) => {
//                                             if (e.target.checked) {
//                                                 setSelectedRows(currentItems.map((item) => item.jebId));
//                                             } else {
//                                                 setSelectedRows([]);
//                                             }
//                                         }}
...
//                                             currentItems.every((item) => selectedRows.includes(item.jebId))
//                                         }
...
//                                         <input
//                                             type="checkbox"
//                                             checked={selectedRows.includes(item.jebId)}
//                                             onChange={() => handleCheckboxChange(item.jebId)}
//                                             className="h-5 w-5 text-blue-500 border-blue-200 rounded focus:ring-blue-400"
//                                         />
//                                     </td>
...
const JoinEditorialBoards = () => {
    ...
    useEffect(() => {
        const fetchEditorialBoardMembers = async () => {
            try {
                const response = await axios.get(`${BASE_URL}/api/v1/joineditorialboard`);
                setEditorialBoardMembers(response.data); // Assuming the API returns an array
            } catch (err) {
                setError("Error fetching editorial board members: " + err.message);
            } finally {
    ...
    const confirmDelete = async () => {
        try {
            setLoading(true);
            const deletePromises = selectedRows.map((id) =>
                axios.delete(`${BASE_URL}/api/v1/joineditorialboard/${id}`).then((response) => {
                    if (response.status !== 200) {
                        throw new Error(`Failed to delete member with ID ${id}`);
                    }
                    return response.data;
    ...
    return (
        <div className="flex min-h-screen bg-blue-50">
            ...
            <div className="w-screen p-6 pt-20">
                {/* Header */}
                <header className="bg-cyan-500 text-white p-4 rounded-t-lg mb-4 shadow-md">
                    <h1 className="text-2xl font-bold">Join Editorial Board Dashboard</h1>
                    <p className="text-sm opacity-90 mt-1">Manage editorial board applications and their details.</p>
                </header>
