...
const Affiliation = () => {
    ...
    // Fetch affiliation records from API
    useEffect(() => {
        const fetchAffiliations = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    setError("Failed to fetch affiliation records");
                }
            } catch (err) {
                setError("Error fetching affiliation records: " + err.message);
            } finally {
                setLoading(false);
            }
        };
    ...
    const filteredAffiliations = affiliations.filter(
        (item) =>
            item.affName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.alternateName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.affCode?.toLowerCase().includes(searchTerm.toLowerCase())
    );

    // Pagination logic
    const totalItems = filteredAffiliations.length;
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const currentItems = filteredAffiliations.slice(startIndex, startIndex + itemsPerPage);
    ...
    const csvHeaders = [
        { label: "S.No", key: "sNo" },
        { label: "Affiliation Name", key: "affName" },
        { label: "Alternate Name", key: "alternateName" },
        { label: "Address", key: "address" },
        { label: "Country", key: "country" },
        { label: "Website", key: "website" },
        { label: "Papers Published", key: "papersPublished" },
        { label: "Affiliation Code", key: "affCode" },
        { label: "Created Date", key: "createdDate" },
        { label: "Status", key: "status" },
    ];

    const csvData = filteredAffiliations.map((item, index) => ({
        sNo: index + 1,
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            pdf.save("affiliation-records.pdf");
        } catch (error) {
    ...
    const confirmDelete = async () => {
        try {
            ...
            const deletePromises = selectedRows.map((id) =>
                axios.delete(`${BASE_URL}/affiliations/${id}`).then((response) => {
                    if (!response.data.success) {
                        throw new Error(`Failed to delete affiliation with ID ${id}`);
                    }
            ...
            setAffiliations((prevAffiliations) =>
                prevAffiliations.filter((aff) => !selectedRows.includes(aff.affId))
            );
            ...
            setSuccess("Affiliation records deleted successfully!");
            setTimeout(() => setSuccess(null), 3000);

            setShowDeleteModal(false);
        } catch (err) {
            setError("Error deleting affiliation records: " + err.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleEdit = () => {
        if (selectedRows.length === 1) {
            const affiliationToEdit = affiliations.find((a) => a.affId === selectedRows[0]);
            navigate("/addnewaffiliation", { state: { affiliation: affiliationToEdit } });
            window.scrollTo(0, 0);
        }
    };
        ...
        <div className="flex min-h-screen bg-blue-50">
            ...
            <div className="w-screen p-6 pt-20">
                ...
                <header className="bg-cyan-500 text-white p-4 rounded-t-lg mb-4 shadow-md">
                    ...
                    <p className="text-sm opacity-90 mt-1">Manage affiliation records and their details.</p>
                </header>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    <div className="flex flex-wrap gap-3">
                        ...
                        <CSVLink
                            data={csvData}
                            headers={csvHeaders}
                            filename={"affiliation-records.csv"}
                    ...
                    <div className="relative w-full sm:w-72">
                        ...
                        <input
                            type="text"
                            placeholder="Search by name, alternate name, or code..."
                            value={searchTerm}
                            ...
                            <tr className="bg-gray-200 text-gray-700 text-sm">
                                <th className="p-3 text-left">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            if (e.target.checked) {
                                                setSelectedRows(currentItems.map((item) => item.affId));
                                            } else {
                                                setSelectedRows([]);
                                            }
                                        }}
                                        checked={
                                            ...
                                            currentItems.every((item) => selectedRows.includes(item.affId))
                                        }
                                        className="h-5 w-5 text-blue-500 border-blue-200 rounded focus:ring-blue-400"
                                    />
                                </th>
                                <th className="p-3 text-left">S.No</th>
                                <th className="p-3 text-left">Affiliation Name</th>
                                <th className="p-3 text-left">Alternate Name</th>
                                <th className="p-3 text-left">Address</th>
                                <th className="p-3 text-left">Country</th>
                                <th className="p-3 text-left">Website</th>
                                <th className="p-3 text-left">Papers Published</th>
                            ...
                            {currentItems.map((item, index) => (
                                ...
                                >
                                    <td className="p-3">
                                        <input
                                            type="checkbox"
                                            checked={selectedRows.includes(item.affId)}
                                            onChange={() => handleCheckboxChange(item.affId)}
                                            className="h-5 w-5 text-blue-500 border-blue-200 rounded focus:ring-blue-400"
                                        />
                                    </td>
                                    <td className="p-3">{startIndex + index + 1}</td>
                                    <td className="p-3">{item.affName}</td>
                                    <td className="p-3">{item.alternateName}</td>
                                    ...
                                    <td className="p-3">{item.papersPublished}</td>
                                    <td className="p-3">{item.affCode}</td>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-6 text-gray-600 text-sm gap-4">
                    <span>
                        ...
                        {totalItems} affiliation records
                    </span>
                    ...
                    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                            <p className="text-lg font-semibold text-gray-700 mb-4">
                                Are you sure you want to delete {selectedRows.length} affiliation record(s)?
                            </p>
