...
const Awards = () => {
        ...
        const fetchAwards = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    setError(response.data.message || "Failed to fetch awards");
                }
            } catch (err) {
    ...
    const filteredAwards = awards.filter((item) =>
        ...
        (item.affiliation || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
        (item.awardType || "").toLowerCase().includes(searchTerm.toLowerCase())
    );

    const totalItems = filteredAwards.length;
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const currentItems = filteredAwards.slice(startIndex, startIndex + itemsPerPage);
    ...
    const csvHeaders = [
        ...
        { label: "ResearchGate", key: "researchgate" },
    ...
    const csvData = filteredAwards.map((item, index) => ({
        sNo: startIndex + index + 1,
        name: item.name || "N/A",
        affiliation: item.affiliation || "N/A",
        email: item.email || "N/A",
        awardType: item.awardType || "N/A",
        content: item.content || "N/A",
        linkedin: item.linkedin || "N/A",
        researchgate: item.researchgate || "N/A",
        academia: item.academia || "N/A",
        googleScholar: item.googleScholar || "N/A",
        profilePhoto: item.profilePhoto || "N/A",
    ...
    const confirmDelete = async () => {
        try {
            ...
            const deletePromises = selectedRows.map((id) =>
                axios.delete(`${BASE_URL}/awards/${id}`).then((response) => {
                    if (!response.data.success) throw new Error(`Failed to delete award with ID ${id}`);
                    return response.data;
                })
            );
            await Promise.all(deletePromises);
            setAwards((prevAwards) => prevAwards.filter((award) => !selectedRows.includes(award.id)));
            setSelectedRows([]);
            if (currentItems.length === selectedRows.length && currentPage > 1) setCurrentPage(currentPage - 1);
            setModal({ show: true, type: "success", message: "Awards deleted successfully!" });
            setTimeout(() => setModal({ show: false, type: "", message: "" }), 3000);
            setShowDeleteModal(false);
        } catch (err) {
            setModal({ show: true, type: "error", message: "Error deleting awards: " + err.message });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleEdit = () => {
        if (selectedRows.length === 1) {
            const awardToEdit = awards.find((a) => a.id === selectedRows[0]);
            navigate("/addnewaward", { state: { award: awardToEdit } });
            window.scrollTo(0, 0);
        }
    };
        ...
        <div className="flex min-h-screen bg-blue-50">
            ...
            <div className="w-screen p-6 pt-20">
                <header className="bg-cyan-500 text-white p-4 rounded-t-lg mb-4 shadow-md">
                    ...
                    <p className="text-sm opacity-90 mt-1">Manage awards and their details.</p>
                </header>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    ...
                    <div className="relative w-full sm:w-72">
                        ...
                        <input
                            type="text"
                            placeholder="Search by name, affiliation, or award type..."
                            value={searchTerm}
                    ...
                    <table ref={tableRef} className="w-full table-auto min-w-[800px]">
                        <thead>
                            <tr className="bg-gray-200 text-gray-700 text-sm">
                                ...
                                <th className="p-3 text-left">Award Type</th>
                        ...
                        <tbody>
                            {currentItems.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="p-3">{item.awardType || "N/A"}</td>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-6 text-gray-600 text-sm gap-4">
                    <span>
                        Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, totalItems)} of {totalItems} awards
                    </span>
                    ...
                    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                            <p className="text-lg font-semibold text-gray-700 mb-4">
                                Are you sure you want to delete {selectedRows.length} award(s)?
                            </p>
