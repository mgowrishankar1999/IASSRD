...
import { useNavigate, Link } from "react-router-dom";
import Sidebar from "../sidebar"; // Adjust path to your Sidebar
import { FaSearch, FaTrash, FaEdit, FaFileCsv, FaFilePdf, FaPrint } from "react-icons/fa";
import { CSVLink } from "react-csv";
import jsPDF from "jspdf";
import html2canvas from "html2canvas";
import axios from "axios";
...
const ArticleSubmission = () => {
    ...
    // Getting role type from localStorage after login
    let roletype = null;
    try {
        const user = localStorage.getItem("user");
        if (user) {
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            rows.forEach((row) => {
                ...
                // Remove links to avoid rendering issues
                const links = row.querySelectorAll("a");
                links.forEach((link) => {
                    const text = document.createTextNode(link.textContent || "View");
    ...
    return (
        <div className="flex min-h-screen bg-blue-50">
            <header className="fixed top-0 left-0 w-full bg-blue-900 text-white py-4 px-6 flex items-center justify-between z-40">
                ...
                <div className="flex items-center space-x-4">
                    <span className="text-sm">Hi {user?.user_name || "User"}</span>
                </div>
            </header>
            <aside className="w-[20%]">
                <Sidebar />
                                ...
                                {currentItems.length === 0 ? (
                                    ...
                                    currentItems.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="p-3 flex gap-2">
                                                {item.uploadManuscript && item.uploadManuscript !== "" && (
                                                    <a
                                                        href={`https://iassrd.com/uploads${item.uploadManuscript}`}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="text-red-600 hover:text-red-800"
                                                        title="View Manuscript"
