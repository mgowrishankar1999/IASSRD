...
//   // Fetch user from localStorage to check role
//   useEffect(() => {
...
//         }
//       } catch (err) {
//         setError(`Error fetching disciplines: ${err.message}`);
//       }
//     };
//     fetchDisciplines();
//   }, []);
...
//       <h2 className="text-2xl font-semibold text-gray-900 mb-6">Disciplines</h2>
//       {error && (
...
//                 <th className="px-4 py-2 text-left text-gray-700">Discipline Name</th>
...
//                   <tr key={discipline.disciplineId} className="border-t">
//                     <td className="px-4 py-2">{discipline.disciplineId}</td>
//                     <td className="px-4 py-2">{discipline.disciplineName}</td>
//                     <td className="px-4 py-2">{discipline.department}</td>
//                   </tr>
//                 ))
//               ) : (
//                 <tr>
...
//     const currentItems = filteredDisciplines.slice(startIndex, startIndex + itemsPerPage);
...
//     const csvHeaders = [
//         { label: "S.No", key: "sNo" },
//         { label: "Discipline Name", key: "disciplineName" },
//         { label: "Department", key: "department" },
//         { label: "Created User ID", key: "createdUserId" },
//         { label: "Created User Type", key: "createdUserType" },
//         { label: "Created Date", key: "createdDate" },
//         { label: "Updated User ID", key: "updatedUserId" },
//         { label: "Updated User Type", key: "updatedUserType" },
//         { label: "Updated Date", key: "updatedDate" },
//         { label: "Status", key: "status" },
//     ];

//     const csvData = filteredDisciplines.map((item, index) => ({
//         sNo: index + 1,
//         disciplineName: item.disciplineName,
//         department: item.department,
//         createdUserId: item.createdUserId,
//         createdUserType: item.createdUserType,
//         createdDate: new Date(item.createdDate).toLocaleString(),
//         updatedUserId: item.updatedUserId,
//         updatedUserType: item.updatedUserType,
...
//             // Create a temporary container to isolate the table
...
//                 axios.delete(`${BASE_URL}/disciplines/${id}`).then((response) => {
//                     if (!response.data.success) {
//                         throw new Error(`Failed to delete discipline with ID ${id}`);
//                     }
...
//                 prevDisciplines.filter((discipline) => !selectedRows.includes(discipline.disciplineId))
//             );
...
//             setSuccess("Disciplines deleted successfully!");
//             setTimeout(() => setSuccess(null), 3000);

//             setShowDeleteModal(false);
//         } catch (err) {
//             setError("Error deleting disciplines: " + err.message);
//         } finally {
//             setLoading(false);
//         }
//     };
...
//             const disciplineToEdit = disciplines.find((d) => d.disciplineId === selectedRows[0]);
//             navigate("/addnewdisciplines", { state: { discipline: disciplineToEdit } });
//             window.scrollTo(0, 0);
//         }
//     };
...
//                 {/* Header */}
//                 <header className="flex justify-between items-center bg-gradient-to-r from-blue-600 to-blue-400 text-white p-4 rounded-xl mb-6 shadow-lg">
//                     <div>
//                         <h1 className="text-2xl font-bold">Discipline Dashboard</h1>
//                         <p className="text-sm opacity-90 mt-1">
//                             Manage disciplines and departments.
//                         </p>
//                     </div>
//                     {/* Logout is already handled in Sidebar */}
//                 </header>
...
//                             className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-full flex items-center gap-2 transition-colors shadow-md"
//                         >
//                             <FaPlus /> Add New
//                         </button>
...
//                                     className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-full flex items-center gap-2 transition-colors shadow-md"
//                                 >
//                                     <FaTrash /> Delete
//                                 </button>
...
//                                     <button
//                                         onClick={handleEdit}
//                                         className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-full flex items-center gap-2 transition-colors shadow-md"
//                                     >
//                                         <FaEdit /> Edit
//                                     </button>
//                                 )}
...
//                             className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-full flex items-center gap-2 transition-colors shadow-md"
//                         >
//                             <FaFilePdf /> Export PDF
//                         </button>
//                         <button
//                             onClick={handlePrint}
//                             className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-full flex items-center gap-2 transition-colors shadow-md"
//                         >
//                             <FaPrint /> Print
//                         </button>
//                     </div>
...
//                     <table ref={tableRef} className="w-full table-auto min-w-[1000px]">
//                         <thead>
...
//                                         onChange={(e) => {
//                                             if (e.target.checked) {
//                                                 setSelectedRows(currentItems.map((item) => item.disciplineId));
//                                             } else {
//                                                 setSelectedRows([]);
//                                             }
//                                         }}
...
//                                             currentItems.every((item) => selectedRows.includes(item.disciplineId))
//                                         }
//                                         className="h-5 w-5 text-blue-500 border-blue-200 rounded focus:ring-blue-400"
//                                     />
//                                 </th>
//                                 <th className="p-3 text-left">S.No</th>
//                                 <th className="p-3 text-left">Discipline Name</th>
//                                 <th className="p-3 text-left">Created User ID</th>
//                                 <th className="p-3 text-left">Created User Type</th>
//                                 <th className="p-3 text-left">Created Date</th>
//                                 <th className="p-3 text-left">Updated User ID</th>
//                                 <th className="p-3 text-left">Updated User Type</th>
...
//                                         <input
//                                             type="checkbox"
//                                             checked={selectedRows.includes(item.disciplineId)}
//                                             onChange={() => handleCheckboxChange(item.disciplineId)}
//                                             className="h-5 w-5 text-blue-500 border-blue-200 rounded focus:ring-blue-400"
//                                         />
//                                     </td>
//                                     <td className="p-3">{startIndex + index + 1}</td>
//                                     <td className="p-3">{item.disciplineName}</td>
//                                     <td className="p-3">{item.createdUserId}</td>
//                                     <td className="p-3">{item.createdUserType}</td>
//                                     <td className="p-3">{new Date(item.createdDate).toLocaleString()}</td>
//                                     <td className="p-3">{item.updatedUserId}</td>
//                                     <td className="p-3">{item.updatedUserType}</td>
//                                     <td className="p-3">{new Date(item.updatedDate).toLocaleString()}</td>
//                                 </tr>
//                             ))}
//                         </tbody>
//                     </table>
...
//                                 Are you sure you want to delete {selectedRows.length} discipline(s)?
//                             </p>
        ...
        <div className="flex min-h-screen bg-blue-50">
            ...
            <div className=" w-screen p-6">
                ...
                <header className="bg-cyan-500 text-white my-15 p-4 rounded-t-lg mb-4 shadow-md">
                    <h1 className="text-2xl font-bold">Discipline Dashboard</h1>
                    <p className="text-sm opacity-90 mt-1">Manage disciplines and departments.</p>
                </header>
