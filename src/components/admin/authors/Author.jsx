...
const Author = () => {
    ...
    const filteredAuthors = authors.filter((item) => {
        const fullName = `${item.prefix || ""} ${item.firstName} ${item.lastName}`.toLowerCase().trim();
        return (
    ...
    const currentItems = filteredAuthors.slice(startIndex, startIndex + itemsPerPage);
    ...
    const csvData = filteredAuthors.map((item, index) => ({
        sNo: index + 1,
    ...
    const confirmDelete = async () => {
        try {
            ...
            const deletePromises = selectedRows.map((id) =>
                axios.delete(`${BASE_URL}/authors/${id}`).then((response) => {
                    if (!response.data.success) {
            ...
            setAuthors((prevAuthors) =>
                prevAuthors.filter((author) => !selectedRows.includes(author.authorId))
            );
            ...
            setModal({ show: true, type: "success", message: "Authors deleted successfully!" });
            setTimeout(() => setModal({ show: false, type: "", message: "" }), 3000);

            setShowDeleteModal(false);
        } catch (err) {
            setModal({ show: true, type: "error", message: "Error deleting authors: " + err.message });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleEdit = () => {
        if (selectedRows.length === 1) {
            const authorToEdit = authors.find((a) => a.authorId === selectedRows[0]);
            navigate("/addnewauthor", { state: { author: authorToEdit } });
            window.scrollTo(0, 0);
        }
    };
        ...
        <div className="flex min-h-screen bg-blue-50">
            ...
            <div className="w-screen p-6 pt-20">
                ...
                <header className="bg-cyan-500 text-white p-4 rounded-t-lg mb-4 shadow-md">
                    <h1 className="text-2xl font-bold">Authors Dashboard</h1>
                    <p className="text-sm opacity-90 mt-1">Manage authors and their details.</p>
                </header>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    ...
                    <div className="relative w-full sm:w-72">
                        ...
                        <input
                            type="text"
                            placeholder="Search by name, designation, department, or university..."
                            value={searchTerm}
                            ...
                            <tr className="bg-gray-200 text-gray-700 text-sm">
                                <th className="p-3 text-left">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            if (e.target.checked) {
                                                setSelectedRows(currentItems.map((item) => item.authorId));
                                            } else {
                                                setSelectedRows([]);
                                            }
                                        }}
                                        checked={
                                            ...
                                            currentItems.every((item) => selectedRows.includes(item.authorId))
                                        }
                            ...
                            {currentItems.map((item, index) => (
                                ...
                                >
                                    <td className="p-3">
                                        <input
                                            type="checkbox"
                                            checked={selectedRows.includes(item.authorId)}
                                            onChange={() => handleCheckboxChange(item.authorId)}
                                            className="h-5 w-5 text-blue-500 border-blue-200 rounded focus:ring-blue-400"
                                        />
                                    </td>
                                    <td className="p-3">{startIndex + index + 1}</td>
                                    <td className="p-3">{`${item.prefix || ""} ${item.firstName} ${item.lastName}`.trim()}</td>
                                    <td className="p-3">{item.designation || "N/A"}</td>
                                    <td className="p-3">{item.department || "N/A"}</td>
                                    <td className="p-3">{item.university || "N/A"}</td>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-6 text-gray-600 text-sm gap-4">
                    <span>
                        Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, totalItems)} of {totalItems} authors
                    </span>
                    ...
                    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                            <p className="text-lg font-semibold text-gray-700 mb-4">
                                Are you sure you want to delete {selectedRows.length} author(s)?
                            </p>
