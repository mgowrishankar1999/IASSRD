...
const AuthorProfile = () => {
    ...
    const [journals, setJournals] = useState({});
    const [disciplines, setDisciplines] = useState({});
    const [authorsMap, setAuthorsMap] = useState({}); // New state for author ID to name mapping
    const [searchQuery, setSearchQuery] = useState('');
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState(null);
    const [isSearchOpen, setIsSearchOpen] = useState(false);
    ...
    return (
        <>
            ...
            <div className={`min-h-screen bg-gray-100 flex flex-col px-[90px] ${isSearchOpen ? 'mt-[130px]' : 'mt-[50px]'}`}>
                <main className="flex-grow md:mx-0 mx-4 mt-28">
                    <div className="bg-white rounded-lg shadow-md p-6">
                        <div className="flex flex-col gap-6">
                            {/* Author Details */}
                            <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                            ...
                            {/* Charts Section */}
                            <div className="flex flex-row font-IBM gap-2">
                                <div className="bg-white p-6 w-[45%] rounded-lg shadow-md">
                                    <h3 className="text-lg font-semibold text-gray-800 mb-4">Papers Published</h3>
                                    <div className="h-48">
                                        <Bar
                                            data={papersPublishedByYearData}
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Volume / Issue</th>
                                            <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Publication Date</th>
                                            <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Co-authors</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-200">
                                        {articles.length > 0 ? (
                                                    ...
                                                    <td className="px-6 py-4 text-sm text-gray-900">
                                                        {article.authorIds
                                                            ? article.authorIds
                                                                .split(",")
                                                                .map((id) => id.trim())
                                                                .filter((id) => id !== author.authorId.toString())
                                                                .map((id) => authorsMap[id] || `Author ID: ${id}`)
                                                                .join(", ") || "None"
                                                            : "None"}
                                                    </td>
                                                </tr>
