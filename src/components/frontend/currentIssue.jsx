...
//                     <Sidebar journalAbbrevation={journalAbbrevation} dynamicProps={journal} />
//                     <div className="w-[75vw] ps-6 h-auto">
//                         <p className="text-3xl text-blue-800 font-bold mb-2">{journal?.journalName}</p>
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (filteredArticles.length === 0) {
                    throw new Error("This issue is currently awaiting publication. New and engaging articles will be available here soon.");
                }
            ...
            <div className={`px-[80px] py-[30px] ${isSearchOpen ? "mt-[140px]" : "mt-[60px]"}`}>
                {loading || contextLoading ? (
                    ...
                    <div className="flex mt-3">
                        <Sidebar journalAbbrevation={journalAbbrevation} dynamicProps={journal} />
                        <div className="w-[75vw] ps-6 h-auto">
                            {error || contextError || !journal || articles.length === 0 ? (
                                <div className="bg-gradient-to-b from-gray-50 to-white min-h-[400px] py-6 px-8">
                                    <p className="text-center text-red-600 text-lg font-medium">
                                        {error || contextError || "This issue is currently awaiting publication. New and engaging articles will be available here soon."}
                                    </p>
                                </div>
                            ) : (
                                <>
                                    ...
                                    <div className="bg-gradient-to-b from-gray-50 to-white min-h-[400px] py-6 px-8">
                                        {articles.map((article, index) => (
                                            <div
                                                key={article.articleId}
                                                className={`pb-6 mt-4 ${index === articles.length - 1 ? "border-b-0" : "border-b border-stone-600"} mb-1`}
                                            >
                                                <div className="flex items-center space-x-3 mb-2">
                                                    <span className="text-sm font-medium text-indigo-600">
                                                        {article.articleType || "Research Article"}
